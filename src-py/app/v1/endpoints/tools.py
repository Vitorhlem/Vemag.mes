...
from app.schemas.tool_schema import ToolCreate, ToolUpdate, ToolPublic
...
@router.get("/management-list", response_model=List[ToolPublic])
...
@router.get("/{implement_id}", response_model=ToolPublic)
...
@router.put("/{implement_id}", response_model=ToolPublic)
async def update_implement(
    *,
    db: AsyncSession = Depends(deps.get_db),
    implement_id: int,
    implement_in: ToolUpdate,
    current_user: User = Depends(deps.get_current_active_manager)
):
    """
